...
# Dar bienvenida, a menos que se haya superado el limite de clientes.
    while True:
        ...
        if len(CONNECTIONS) > CLIENTLIMIT: # si se llega al limite, no darle la bienvenida
            ...
            client.send(bytes("Greetings fellow comrade to this totally legit human chat!", "utf8"))
...
# Permitir que el cliente interactue en el chat, a menos que se haya superado el limite de clientes.
    ...
    if len(CONNECTIONS) > CLIENTLIMIT: #  no hacerle mas pregunta ni dejarlo interactuar con los demas en el chat
        ...
        client.send(bytes("You have joined the quantum chat!", 'UTF-8'))
        # informar a los demas que llego un nuevo usuario
        ...
        while True:
            ...
            if messages == bytes(":q", 'UTF-8'): # cerrar sesion
            ...
            elif messages == bytes(":u", 'UTF-8'): # mostrar clientes activos
                ...
                if check and try_check_balance(check, client, identifier):
                    ...
                    try: 
                        int(string_amount) # probar que el monto sea un entero
...
# Entregar lista de usuarios al cliente
    
    client.send(bytes("Currently active users:", 'UTF-8')) 
    
    for i in NAMES:   
        client.send(bytes('<' + NAMES[i] + '>', 'UTF-8')) #enviar el nombre de usuario al cliente
...
ADDRESSES = {} # dirreciones de los clientes
NAMES = {} #nombres de los clientes
BALANCES = {} # saldos de los clientes
...
SERVER.bind((HOST, PORT)) # enlazar el servidor con la tupla de host y puerto

if __name__ == "__main__":
    SERVER.listen(6) # estar atento a 6 conexiones
