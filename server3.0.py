# -*- coding: utf-8 -*-
"""
Created on Mon May  6 18:29:10 2019
...
# revisar sintaxis y enviar el mensaje privado si lo cumple
    ...
    if msgstring.startswith(":p - <"):
        ...
        if ind == 0: # formato incorrecto
            ...
            dest_str = msgstring[6:ind]
            dest = names_client(dest_str) # direccion destinatario
            
            if dest == client:
                client.send(bytes("Don't send privates messages to yourself. Be social and talk to other people!", 'UTF-8'))
                
            else:
                pmsg = msgstring[ind+4:] # mensaje
                pmsg = replace_emojis(pmsg) # reemplazar emojis
                dest.send(bytes('<'+str(name)+'> (private): '+str(pmsg), 'UTF-8'))
                client.send(bytes('Private message: "' +str(pmsg) +'" has been sent to : '+'<'+str(dest_str)+'> ', 'UTF-8'))
...
# Realizar transferencia
    ...
    if saldo < amount:
        ...
        if target == client:
            client.send(bytes("You can't transfer money to yourself!", 'UTF-8'))
